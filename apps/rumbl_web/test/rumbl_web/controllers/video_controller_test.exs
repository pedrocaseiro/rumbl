defmodule Rumbo.VideoControllerTest do
  use RumblWeb.ConnCase

  alias Rumbl.Multimedia
  @create_attrs %{url: "http://bota", title: "bota", description: "a bota video"}
  ...
  test "authorizes actions against access by other users", %{conn: conn} do
    owner = user_fixture(username: "jon")
    video = video_fixture(owner, title: "funny cats")
    non_owner = user_fixture(username: "david")
    ...
    assert_error_sent :not_found, fn ->
      get(conn, Routes.video_path(conn, :update, video, @create_attrs))
    end
  ...
  describe "with a logged in user" do
    ...
    test "lists all user's videos on index", %{conn: conn, user: user} do
      ...
      other_video = video_fixture(user_fixture(username: "another user"), title: "funny dogs")
    ...
    @tag login_as: "max"
    test "does not create video and renders when invalid", %{conn: conn, user: user} do
